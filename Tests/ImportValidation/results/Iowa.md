# Iowa: IFC import

- Imported: `results/Iowa.pgs`
- No expected values - this shows which values the import set

Alignment, profile, and deck edges are evaluated at at[0] = -0+01.50, at[1] = 0+21.50, at[2] = 0+44.50, at[3] = 0+98.00, at[4] = 1+51.50, at[5] = 1+74.50, at[6] = 1+97.50

**136 of 260 values set by the import**, 124 are template defaults

## template default (124)

| Value | Template | Imported |
|---|---|---|
| `bridge.girder_family` | I-Beam | I-Beam |
| `bridge.girder_orientation` | 0 | 0 |
| `bridge.same_girder_for_entire_bridge` | -1 | -1 |
| `bridge.girder_spacing_type` | 0 | 0 |
| `bridge.fillet` | 0.750 in | 0.750 in |
| `pier[0].connection_type` | 1 | 1 |
| `pier[0].back.end_distance_measure` | FromBearingNormalToPier | FromBearingNormalToPier |
| `pier[0].back.bearing_offset_measure` | NormalToPier | NormalToPier |
| `pier[0].ahead.bearing.shape` | 0 | 0 |
| `pier[0].ahead.bearing.count` | 1 | 1 |
| `pier[0].ahead.end_distance_measure` | FromBearingNormalToPier | FromBearingNormalToPier |
| `pier[0].ahead.bearing_offset_measure` | NormalToPier | NormalToPier |
| `pier[1].connection_type` | 1 | 6 |
| `pier[1].back.bearing.shape` | 0 | 0 |
| `pier[1].back.bearing.count` | 1 | 1 |
| `pier[1].back.end_distance_measure` | FromBearingNormalToPier | FromBearingNormalToPier |
| `pier[1].back.bearing_offset_measure` | NormalToPier | NormalToPier |
| `pier[1].ahead.bearing.shape` | 0 | 0 |
| `pier[1].ahead.bearing.count` | 1 | 1 |
| `pier[1].ahead.end_distance_measure` | FromBearingNormalToPier | FromBearingNormalToPier |
| `pier[1].ahead.bearing_offset_measure` | NormalToPier | NormalToPier |
| `pier[2].connection_type` | 1 | 6 |
| `pier[2].back.bearing.shape` | 0 | 0 |
| `pier[2].back.bearing.count` | 1 | 1 |
| `pier[2].back.end_distance_measure` | FromBearingNormalToPier | FromBearingNormalToPier |
| `pier[2].back.bearing_offset_measure` | NormalToPier | NormalToPier |
| `pier[2].ahead.bearing.shape` | 0 | 0 |
| `pier[2].ahead.bearing.count` | 1 | 1 |
| `pier[2].ahead.end_distance_measure` | FromBearingNormalToPier | FromBearingNormalToPier |
| `pier[2].ahead.bearing_offset_measure` | NormalToPier | NormalToPier |
| `pier[3].connection_type` | 1 | 6 |
| `pier[3].back.bearing.shape` | 0 | 0 |
| `pier[3].back.bearing.count` | 1 | 1 |
| `pier[3].back.end_distance_measure` | FromBearingNormalToPier | FromBearingNormalToPier |
| `pier[3].back.bearing_offset_measure` | NormalToPier | NormalToPier |
| `pier[3].ahead.end_distance_measure` | FromBearingNormalToPier | FromBearingNormalToPier |
| `pier[3].ahead.bearing_offset_measure` | NormalToPier | NormalToPier |
| `group[0].piers` | 0-1 | 0-1 |
| `group[0].girder[0].type` | Unknown_Girder_Type | Unknown_Girder_Type |
| `group[0].girder[0].fc` | 6.000 ksi | 6.000 ksi |
| `group[0].girder[0].fci` | 5.000 ksi | 5.000 ksi |
| `group[0].girder[1].type` | Unknown_Girder_Type | Unknown_Girder_Type |
| `group[0].girder[1].fc` | 6.000 ksi | 6.000 ksi |
| `group[0].girder[1].fci` | 5.000 ksi | 5.000 ksi |
| `group[0].girder[2].type` | Unknown_Girder_Type | Unknown_Girder_Type |
| `group[0].girder[2].fc` | 6.000 ksi | 6.000 ksi |
| `group[0].girder[2].fci` | 5.000 ksi | 5.000 ksi |
| `group[0].girder[3].type` | Unknown_Girder_Type | Unknown_Girder_Type |
| `group[0].girder[3].fc` | 6.000 ksi | 6.000 ksi |
| `group[0].girder[3].fci` | 5.000 ksi | 5.000 ksi |
| `group[0].girder[4].type` | Unknown_Girder_Type | Unknown_Girder_Type |
| `group[0].girder[4].fc` | 6.000 ksi | 6.000 ksi |
| `group[0].girder[4].fci` | 5.000 ksi | 5.000 ksi |
| `group[0].girder[5].type` | Unknown_Girder_Type | Unknown_Girder_Type |
| `group[0].girder[5].fc` | 6.000 ksi | 6.000 ksi |
| `group[0].girder[5].fci` | 5.000 ksi | 5.000 ksi |
| `group[0].girder[6].type` | Unknown_Girder_Type | Unknown_Girder_Type |
| `group[0].girder[6].fc` | 6.000 ksi | 6.000 ksi |
| `group[0].girder[6].fci` | 5.000 ksi | 5.000 ksi |
| `group[1].girder[0].type` | Unknown_Girder_Type | Unknown_Girder_Type |
| `group[1].girder[0].fc` | 6.000 ksi | 6.000 ksi |
| `group[1].girder[0].fci` | 5.000 ksi | 5.000 ksi |
| `group[1].girder[1].type` | Unknown_Girder_Type | Unknown_Girder_Type |
| `group[1].girder[1].fc` | 6.000 ksi | 6.000 ksi |
| `group[1].girder[1].fci` | 5.000 ksi | 5.000 ksi |
| `group[1].girder[2].type` | Unknown_Girder_Type | Unknown_Girder_Type |
| `group[1].girder[2].fc` | 6.000 ksi | 6.000 ksi |
| `group[1].girder[2].fci` | 5.000 ksi | 5.000 ksi |
| `group[1].girder[3].type` | Unknown_Girder_Type | Unknown_Girder_Type |
| `group[1].girder[3].fc` | 6.000 ksi | 6.000 ksi |
| `group[1].girder[3].fci` | 5.000 ksi | 5.000 ksi |
| `group[1].girder[4].type` | Unknown_Girder_Type | Unknown_Girder_Type |
| `group[1].girder[4].fc` | 6.000 ksi | 6.000 ksi |
| `group[1].girder[4].fci` | 5.000 ksi | 5.000 ksi |
| `group[1].girder[5].type` | Unknown_Girder_Type | Unknown_Girder_Type |
| `group[1].girder[5].fc` | 6.000 ksi | 6.000 ksi |
| `group[1].girder[5].fci` | 5.000 ksi | 5.000 ksi |
| `group[1].girder[6].type` | Unknown_Girder_Type | Unknown_Girder_Type |
| `group[1].girder[6].fc` | 6.000 ksi | 6.000 ksi |
| `group[1].girder[6].fci` | 5.000 ksi | 5.000 ksi |
| `group[2].girder[0].type` | Unknown_Girder_Type | Unknown_Girder_Type |
| `group[2].girder[0].fc` | 6.000 ksi | 6.000 ksi |
| `group[2].girder[0].fci` | 5.000 ksi | 5.000 ksi |
| `group[2].girder[1].type` | Unknown_Girder_Type | Unknown_Girder_Type |
| `group[2].girder[1].fc` | 6.000 ksi | 6.000 ksi |
| `group[2].girder[1].fci` | 5.000 ksi | 5.000 ksi |
| `group[2].girder[2].type` | Unknown_Girder_Type | Unknown_Girder_Type |
| `group[2].girder[2].fc` | 6.000 ksi | 6.000 ksi |
| `group[2].girder[2].fci` | 5.000 ksi | 5.000 ksi |
| `group[2].girder[3].type` | Unknown_Girder_Type | Unknown_Girder_Type |
| `group[2].girder[3].fc` | 6.000 ksi | 6.000 ksi |
| `group[2].girder[3].fci` | 5.000 ksi | 5.000 ksi |
| `group[2].girder[4].type` | Unknown_Girder_Type | Unknown_Girder_Type |
| `group[2].girder[4].fc` | 6.000 ksi | 6.000 ksi |
| `group[2].girder[4].fci` | 5.000 ksi | 5.000 ksi |
| `group[2].girder[5].type` | Unknown_Girder_Type | Unknown_Girder_Type |
| `group[2].girder[5].fc` | 6.000 ksi | 6.000 ksi |
| `group[2].girder[5].fci` | 5.000 ksi | 5.000 ksi |
| `group[2].girder[6].type` | Unknown_Girder_Type | Unknown_Girder_Type |
| `group[2].girder[6].fc` | 6.000 ksi | 6.000 ksi |
| `group[2].girder[6].fci` | 5.000 ksi | 5.000 ksi |
| `deck.type` | 0 | 0 |
| `deck.fc` | 4.000 ksi | 4.000 ksi |
| `deck.edge_point_count` | 1 | 1 |
| `railing.left.exterior` | 42&dq; Single Slope | 42&dq; Single Slope |
| `railing.right.exterior` | 42&dq; Single Slope | 42&dq; Single Slope |
| `pier[0].skew` | 0.0000 deg | 0.0000 deg |
| `pier[1].skew` | 0.0000 deg | 0.0000 deg |
| `pier[2].skew` | 0.0000 deg | 0.0000 deg |
| `pier[3].skew` | 0.0000 deg | 0.0000 deg |
| `at[0].profile.elevation` | 0.000 ft | 0.000 ft |
| `at[0].profile.grade` | 0.0000 % | 0.0000 % |
| `at[1].profile.elevation` | 0.000 ft | 0.000 ft |
| `at[1].profile.grade` | 0.0000 % | 0.0000 % |
| `at[2].profile.elevation` | 0.000 ft | 0.000 ft |
| `at[2].profile.grade` | 0.0000 % | 0.0000 % |
| `at[3].profile.elevation` | 0.000 ft | 0.000 ft |
| `at[3].profile.grade` | 0.0000 % | 0.0000 % |
| `at[4].profile.elevation` | 0.000 ft | 0.000 ft |
| `at[4].profile.grade` | 0.0000 % | 0.0000 % |
| `at[5].profile.elevation` | 0.000 ft | 0.000 ft |
| `at[5].profile.grade` | 0.0000 % | 0.0000 % |
| `at[6].profile.elevation` | 0.000 ft | 0.000 ft |
| `at[6].profile.grade` | 0.0000 % | 0.0000 % |

## set by the import (136)

| Value | Template | Imported |
|---|---|---|
| `bridge.girder_spacing` | 6.000 ft (type 0, location 0) | 6.694 ft (type 0, location 0) |
| `bridge.slab_offset_type` | 0 | 2 |
| `bridge.slab_offset` | 11.000 in |  |
| `bridge.pier_count` | 2 | 4 |
| `pier[0].station` | 0+00.00 | -0+01.50 |
| `pier[0].back.end_distance` | 20.500 in | 8.000 in |
| `pier[0].back.bearing_offset` | 32.500 in | 0.000 in |
| `pier[0].ahead.bearing.length` | 12.000 in | 3.000 in |
| `pier[0].ahead.bearing.width` | 12.000 in | 28.500 in |
| `pier[0].ahead.bearing.height` | 0.000 in | 3.000 in |
| `pier[0].ahead.bearing.fixed_x` | 0 | -1 |
| `pier[0].ahead.bearing.fixed_y` | 0 | -1 |
| `pier[0].ahead.end_distance` | 20.500 in | 8.000 in |
| `pier[0].ahead.bearing_offset` | 32.500 in | 0.000 in |
| `pier[1].station` | 0+00.00 | 0+44.50 |
| `pier[1].back.bearing.length` | 12.000 in | 9.000 in |
| `pier[1].back.bearing.width` | 12.000 in | 28.500 in |
| `pier[1].back.bearing.height` | 0.000 in | 1.000 in |
| `pier[1].back.bearing.fixed_x` | 0 | -1 |
| `pier[1].back.bearing.fixed_y` | 0 | -1 |
| `pier[1].back.end_distance` | 20.500 in | 8.000 in |
| `pier[1].back.bearing_offset` | 32.500 in | 12.000 in |
| `pier[1].ahead.bearing.length` | 12.000 in | 9.000 in |
| `pier[1].ahead.bearing.width` | 12.000 in | 28.500 in |
| `pier[1].ahead.bearing.height` | 0.000 in | 1.000 in |
| `pier[1].ahead.bearing.fixed_x` | 0 | -1 |
| `pier[1].ahead.bearing.fixed_y` | 0 | -1 |
| `pier[1].ahead.end_distance` | 20.500 in | 8.000 in |
| `pier[1].ahead.bearing_offset` | 32.500 in | 12.000 in |
| `pier[2].station` | 0+00.00 | 1+51.50 |
| `pier[2].back.bearing.length` | 12.000 in | 9.000 in |
| `pier[2].back.bearing.width` | 12.000 in | 28.500 in |
| `pier[2].back.bearing.height` | 0.000 in | 1.000 in |
| `pier[2].back.bearing.fixed_x` | 0 | -1 |
| `pier[2].back.bearing.fixed_y` | 0 | -1 |
| `pier[2].back.end_distance` | 20.500 in | 8.000 in |
| `pier[2].back.bearing_offset` | 32.500 in | 12.000 in |
| `pier[2].ahead.bearing.length` | 12.000 in | 9.000 in |
| `pier[2].ahead.bearing.width` | 12.000 in | 28.500 in |
| `pier[2].ahead.bearing.height` | 0.000 in | 1.000 in |
| `pier[2].ahead.bearing.fixed_x` | 0 | -1 |
| `pier[2].ahead.bearing.fixed_y` | 0 | -1 |
| `pier[2].ahead.end_distance` | 20.500 in | 8.000 in |
| `pier[2].ahead.bearing_offset` | 32.500 in | 12.000 in |
| `pier[3].station` | 0+00.00 | 1+97.50 |
| `pier[3].back.bearing.length` | 12.000 in | 3.000 in |
| `pier[3].back.bearing.width` | 12.000 in | 28.500 in |
| `pier[3].back.bearing.height` | 0.000 in | 3.000 in |
| `pier[3].back.bearing.fixed_x` | 0 | -1 |
| `pier[3].back.bearing.fixed_y` | 0 | -1 |
| `pier[3].back.end_distance` | 20.500 in | 8.000 in |
| `pier[3].back.bearing_offset` | 32.500 in | -0.000 in |
| `pier[3].ahead.end_distance` | 20.500 in | 8.000 in |
| `pier[3].ahead.bearing_offset` | 32.500 in | -0.000 in |
| `bridge.group_count` | 1 | 3 |
| `group[0].girder_count` | 5 | 7 |
| `group[0].girder[0].slab_offset_start` | 11.000 in | 9.025 in |
| `group[0].girder[0].slab_offset_end` | 11.000 in | 10.585 in |
| `group[0].girder[1].slab_offset_start` | 11.000 in | 9.025 in |
| `group[0].girder[1].slab_offset_end` | 11.000 in | 10.585 in |
| `group[0].girder[2].slab_offset_start` | 11.000 in | 9.025 in |
| `group[0].girder[2].slab_offset_end` | 11.000 in | 10.585 in |
| `group[0].girder[3].slab_offset_start` | 11.000 in | 8.663 in |
| `group[0].girder[3].slab_offset_end` | 11.000 in | 10.267 in |
| `group[0].girder[4].slab_offset_start` | 11.000 in | 9.025 in |
| `group[0].girder[4].slab_offset_end` | 11.000 in | 10.585 in |
| `group[0].girder[5].slab_offset_start` | 11.000 in | 9.025 in |
| `group[0].girder[5].slab_offset_end` | 11.000 in | 10.585 in |
| `group[0].girder[6].slab_offset_start` | 11.000 in | 9.025 in |
| `group[0].girder[6].slab_offset_end` | 11.000 in | 10.585 in |
| `group[1].piers` | 0-1 | 1-2 |
| `group[1].girder_count` | 5 | 7 |
| `group[1].girder[0].slab_offset_start` | 11.000 in | 10.765 in |
| `group[1].girder[0].slab_offset_end` | 11.000 in | 10.601 in |
| `group[1].girder[1].slab_offset_start` | 11.000 in | 10.765 in |
| `group[1].girder[1].slab_offset_end` | 11.000 in | 10.601 in |
| `group[1].girder[2].slab_offset_start` | 11.000 in | 10.765 in |
| `group[1].girder[2].slab_offset_end` | 11.000 in | 10.601 in |
| `group[1].girder[3].slab_offset_start` | 11.000 in | 10.449 in |
| `group[1].girder[3].slab_offset_end` | 11.000 in | 10.318 in |
| `group[1].girder[4].slab_offset_start` | 11.000 in | 10.765 in |
| `group[1].girder[4].slab_offset_end` | 11.000 in | 10.601 in |
| `group[1].girder[5].slab_offset_start` | 11.000 in | 10.765 in |
| `group[1].girder[5].slab_offset_end` | 11.000 in | 10.601 in |
| `group[1].girder[6].slab_offset_start` | 11.000 in | 10.765 in |
| `group[1].girder[6].slab_offset_end` | 11.000 in | 10.601 in |
| `group[2].piers` | 0-1 | 2-3 |
| `group[2].girder_count` | 5 | 7 |
| `group[2].girder[0].slab_offset_start` | 11.000 in | 10.746 in |
| `group[2].girder[0].slab_offset_end` | 11.000 in | 8.992 in |
| `group[2].girder[1].slab_offset_start` | 11.000 in | 10.746 in |
| `group[2].girder[1].slab_offset_end` | 11.000 in | 8.992 in |
| `group[2].girder[2].slab_offset_start` | 11.000 in | 10.746 in |
| `group[2].girder[2].slab_offset_end` | 11.000 in | 8.992 in |
| `group[2].girder[3].slab_offset_start` | 11.000 in | 10.456 in |
| `group[2].girder[3].slab_offset_end` | 11.000 in | 8.596 in |
| `group[2].girder[4].slab_offset_start` | 11.000 in | 10.746 in |
| `group[2].girder[4].slab_offset_end` | 11.000 in | 8.992 in |
| `group[2].girder[5].slab_offset_start` | 11.000 in | 10.746 in |
| `group[2].girder[5].slab_offset_end` | 11.000 in | 8.992 in |
| `group[2].girder[6].slab_offset_start` | 11.000 in | 10.746 in |
| `group[2].girder[6].slab_offset_end` | 11.000 in | 8.992 in |
| `deck.gross_depth` | 7.500 in | 8.470 in |
| `deck.left_overhang_edge_depth` | 7.000 in | 10.059 in |
| `deck.right_overhang_edge_depth` | 7.000 in | 9.942 in |
| `deck.left_overhang_taper` | 1 | 2 |
| `deck.right_overhang_taper` | 1 | 2 |
| `deck.haunch_shape` | 1 | 0 |
| `at[0].alignment.point` | (-1.500, 0.000) ft | (16590744.323, 6958091.803) ft |
| `at[0].alignment.direction` | 0.0000 deg | 89.9995 deg |
| `at[0].deck.left_edge` | 15.000 ft | 23.583 ft |
| `at[0].deck.right_edge` | 15.000 ft | 23.583 ft |
| `at[1].alignment.point` | (21.500, 0.000) ft | (16590744.323, 6958114.803) ft |
| `at[1].alignment.direction` | 0.0000 deg | 89.9995 deg |
| `at[1].deck.left_edge` | 15.000 ft | 23.583 ft |
| `at[1].deck.right_edge` | 15.000 ft | 23.583 ft |
| `at[2].alignment.point` | (44.500, 0.000) ft | (16590744.323, 6958137.803) ft |
| `at[2].alignment.direction` | 0.0000 deg | 89.9995 deg |
| `at[2].deck.left_edge` | 15.000 ft | 23.583 ft |
| `at[2].deck.right_edge` | 15.000 ft | 23.583 ft |
| `at[3].alignment.point` | (98.000, 0.000) ft | (16590744.324, 6958191.303) ft |
| `at[3].alignment.direction` | 0.0000 deg | 89.9995 deg |
| `at[3].deck.left_edge` | 15.000 ft | 23.583 ft |
| `at[3].deck.right_edge` | 15.000 ft | 23.583 ft |
| `at[4].alignment.point` | (151.500, 0.000) ft | (16590744.324, 6958244.803) ft |
| `at[4].alignment.direction` | 0.0000 deg | 89.9995 deg |
| `at[4].deck.left_edge` | 15.000 ft | 23.583 ft |
| `at[4].deck.right_edge` | 15.000 ft | 23.583 ft |
| `at[5].alignment.point` | (174.500, 0.000) ft | (16590744.324, 6958267.803) ft |
| `at[5].alignment.direction` | 0.0000 deg | 89.9995 deg |
| `at[5].deck.left_edge` | 15.000 ft | 23.583 ft |
| `at[5].deck.right_edge` | 15.000 ft | 23.583 ft |
| `at[6].alignment.point` | (197.500, 0.000) ft | (16590744.325, 6958290.803) ft |
| `at[6].alignment.direction` | 0.0000 deg | 89.9995 deg |
| `at[6].deck.left_edge` | 15.000 ft | 23.583 ft |
| `at[6].deck.right_edge` | 15.000 ft | 23.583 ft |
