REM - Script to prepare for Release

SET BINTARGET=bin
SET REGFREECOM=%ARPDIR%\BridgeLink\RegFreeCOM


REM - Experimental Extensions
xcopy /Y /d %REGFREECOM%\x64\Release\PGSuperIfcExtensions.dll	%BINTARGET%\Extensions\Experimental\x64\
xcopy /Y /d %REGFREECOM%\x64\Release\IfcExtensions.Manifest.PGSuper	%BINTARGET%\Extensions\Experimental\x64\
xcopy /Y /d %REGFREECOM%\x64\Release\IfcImportTemplate.pgt	%BINTARGET%\Extensions\Experimental\x64\

