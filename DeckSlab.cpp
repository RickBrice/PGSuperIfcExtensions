...
std::map<double, std::pair<double, double>> condense(const std::multimap<double, double>& mm)
{
   ...
   // if this condenses to zero, this isn't going to work for PGSuper
   // as a stopgap measure, assume equal offsets on both sides of the station
   // the root of this problem is the skew in the deck profiles from IfcSectionedSolidHorizontal
   // generated by PGSuper IFC Export
   if (result.size() == 0)
   {
      for (auto [station,offset] : mm)
      {
         double left_offset = offset;
         double right_offset = -1. * offset;
         if (right_offset < left_offset) std::swap(left_offset, right_offset);
         result[station] = std::make_pair(left_offset, right_offset);
      }
   }
